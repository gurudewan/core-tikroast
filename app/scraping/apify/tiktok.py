...
def do_profile_scrape(usernames):
    client = ApifyClient(consts.APIFY_API_KEY)

    run_input = { "profiles": usernames, "resultsPerPage": consts.POSTS_PER_PROFILE, "excludePinnedPosts": False, }
...
def do_comments_scrape(post_urls):
    client = ApifyClient(consts.APIFY_API_KEY)
    run_input = { "postURLs": post_urls, "commentsPerPost": consts.COMMENTS_PER_POST }
